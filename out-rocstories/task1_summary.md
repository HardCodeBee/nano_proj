/**
 * File role:
 * This is the short, report-friendly summary of the current best Task 1 run.
...
Train a nanoGPT model from scratch on ROCStories for short story generation, staying within the 32M parameter budget. This summary reflects the current best Task 1 tuning round after iteratively improving the original baseline and the first optimization rerun.
...
- Training run: ~59 minutes on the RTX 4060 Laptop GPU
...
- Step 3200: val loss 3.4336
- Step 4200: val loss 3.3731
- Step 4800: val loss 3.3419
- Best sampled validation estimate from checkpoint metadata: 3.3411 at step 5000

Note: the training run completed to 8000 steps, but the best checkpoint remained the one saved at step 5000.
...
- Average loss: `3.364 -> 3.299`
- Perplexity: `28.89 -> 27.09`

Improvement over the first optimization rerun:

- Average loss: `3.318 -> 3.299`
- Perplexity: `27.60 -> 27.09`
...
- Strengths:
  ...
  - The current tuning round improved validation loss and full-set perplexity again without changing model size.
...
- Evaluation log: `out-rocstories/eval_tuned_round2.log`
