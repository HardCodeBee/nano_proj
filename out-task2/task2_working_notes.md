...
- A later from-scratch pivot, `tiny_roc_curriculum_v2`, also failed to convert its cleaner story-shape sample summary into a real win: Stage B landed at `avg_loss 3.3610`, `ppl 28.83`, `mean_qwen_score 1.9`, so it did not clear the gate for Stage C.
...
The `tiny_roc_curriculum_v2` pivot should also stay out of the priority queue for now: Stage A looked somewhat more ROC-shaped on a tiny sample summary, but the recorded Stage B result was too weak to justify Stage C.
