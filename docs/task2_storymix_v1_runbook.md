...
Its current scoreboard outcome is:

- Stage A: `ppl 27.19`, `judge 2.0`
- Stage B: `ppl 27.70`, `judge 2.0`

Stage B regressed relative to Stage A, and both runs still trail the current
repo bar `e4-posteot-mask-openai`. Because of that, `storymix_v1 Stage C`
should not continue on the active path. The new default direction is
`tiny_roc_curriculum_v2`.
...
- The branch remains useful as a historical comparison.
- The branch is not the current winner.
